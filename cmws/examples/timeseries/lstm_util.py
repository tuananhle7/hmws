...
class TimeseriesDistributionDiscrete:
    ...
    def __init__(
        self, num_classes, embedding, lstm, linear, lstm_eos, max_num_timesteps=200,
    ):
    ...
    def sample(self, sample_shape=torch.Size(), num_timesteps=None, warning_enabled=True):
        ...
        lstm_input = torch.cat(
            [
                torch.zeros((num_samples * self.batch_size, self.num_classes), device=self.device),
                embedding_expanded,
            ],
            dim=1,
        )
    ...
    def log_prob(self, x, eos):
        ...
        for batch_id in range(batch_size):
            ...
            # [num_timesteps_b, num_classes + embedding_dim]
        ...
        # [max_num_timesteps, batch_size, num_classes + 1 or num_classes]
...
class TimeseriesDistributionContinuous:
